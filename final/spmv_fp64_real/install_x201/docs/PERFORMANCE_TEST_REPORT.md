...
This report presents comprehensive performance and accuracy validation results for the `spmv_fp64_real` library on **Mars X201** (国产GPU, Warp=64).

**Key Findings**:
- ✅ Optimized library achieves **3.2x speedup** vs nctigpu (spmv_1)
- ✅ **776 GB/s bandwidth** vs 242 GB/s for nctigpu
- ✅ **42% utilization** vs 13% for nctigpu
- ✅ All accuracy tests PASSED (max error ~1.6e-24)

---

## Real Test Cases Performance

### Comparison: nctigpu (spmv_1) vs Optimized spmv_fp64

Tested on 7 real cases from `/home/chenbinxiangc/spmv_muxi/real_cases/mtx/`

| Case | Rows | NNZ | avgNnz | nctigpu(ms) | opt(ms) | ncti BW | opt BW | Speedup |
|------|------|-----|--------|-------------|---------|---------|--------|---------|
| p0 | 1,256,923 | 13,465,911 | 10.7 | 1.15 | **0.36** | 242 | **776** | **3.21x** |
| p1 | 1,256,923 | 13,465,911 | 10.7 | 1.15 | **0.36** | 242 | **777** | **3.21x** |
| p2 | 1,256,923 | 13,465,911 | 10.7 | 1.15 | **0.36** | 775 | **775** | **3.21x** |
| p3 | 1,256,923 | 13,465,911 | 10.7 | 1.15 | **0.36** | 242 | **776** | **3.21x** |
| p4 | 1,256,923 | 13,465,911 | 10.7 | 1.15 | **0.36** | 242 | **775** | **3.20x** |
| p5 | 1,256,923 | 13,465,911 | 10.7 | 1.15 | **0.36** | 242 | **774** | **3.20x** |
| p6 | 1,256,923 | 13,465,911 | 10.7 | 1.15 | **0.36** | 242 | **776** | **3.21x** |

**Average**: nctigpu 242 GB/s, Optimized **776 GB/s**, Speedup **3.2x**

### Summary Table

| Metric | nctigpu (spmv_1) | Optimized spmv_fp64 | Improvement |
|--------|------------------|---------------------|-------------|
| Kernel Time | 1.15 ms | **0.36 ms** | **3.2x faster** |
| Bandwidth | 242 GB/s | **776 GB/s** | **+320%** |
| Utilization | 13% | **42%** | **+229%** |
| Accuracy | Reference | Identical | Max err ~1.6e-24 |
...
For test cases with avgNnz~10.7, TPR=8 is selected (8 threads per row, 8 rows per warp).

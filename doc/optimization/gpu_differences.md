...
## 文档信息
- **创建日期**: 2026-04-10
- **目标**: 记录国产GPU (Mars X201) 与 NVIDIA RTX 4090 的开发差异
...
|------|-----------|----------|----------|
| `sm_xx` 架构指定 | ❌ 不支持 | ✅ 支持 | 不指定，让编译器处理 |
| `CUDA_VERSION` 宏 | ❌ 不支持 | ✅ 支持 | 条件编译或运行时检测 |
| `printf` in kernel | ⚠️ 有问题 | ✅ 支持 | 使用日志库 |
...
```cpp
// Mars X201 (Warp=64)的Warp Shuffle
template<int TPR>
__device__ double warpReduce(double val) {
    // 对于64线程warp，需要64位mask
    if (TPR >= 64) {

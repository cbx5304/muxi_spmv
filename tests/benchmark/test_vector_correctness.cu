/**
 * @file test_vector_vs_scalar_correctness.cu
 * @brief 验证Vector kernel的正确性并与Simple Scalar对比性能
 *
 * 包含正确性验证和性能测试
 */
...
int main(int argc, char** argv) {
    ...
    printf("========================================\n");
    printf("  Vector vs Scalar Correctness & Performance\n");
    printf("  FP64, blockSize=128\n");
    printf("========================================\n\n");
    ...
    for (int i = 0; i < numRows; i++) {
        ...
        double maxAbs = fmax(fabs(h_y_scalar[i]), fabs(h_y_vector[i]));
        if (diff > maxDiff) maxDiff = diff;
        if (maxAbs > maxVal) maxVal = maxAbs;
        if (diff > 1e-10) diffCount++;
    }

    printf("Max difference: %.15e\n", maxDiff);
    printf("Max value: %.6f\n", maxVal);
    printf("Rows with difference > 1e-10: %d / %d\n", diffCount, numRows);

    if (maxDiff < 1e-10) {
        printf("CORRECTNESS: PASSED ✓\n");
    } else {
        printf("CORRECTNESS: FAILED (diff=%.2e)\n", maxDiff);
    }

    // Performance test
    printf("\nPerformance test (50 iterations each)...\n");
    ...
    printf("  hcTracer --hctx ./test_vector_correctness\n");
    printf("========================================\n");

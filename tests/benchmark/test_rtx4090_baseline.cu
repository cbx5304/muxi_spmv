/**
 * @file test_rtx4090_baseline.cu
 * @brief RTX 4090基线性能测试 - 使用cudaEvent计时
 */
...
void testMatrix(const char* mtxFile, int testIters) {
    ...
    cudaEventElapsedTime(&origTime, start, stop);
    ...
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&rcmTime, start, stop);

    // Calculate performance
    float origAvgUs = origTime * 1000 / testIters;  // us per iteration
    float rcmAvgUs = rcmTime * 1000 / testIters;

    // Bandwidth calculation (effective data transfer)
    // values: nnz * 8, colIdx: nnz * 4, x: nnz * 8 (accessed via colIdx), y: numRows * 8
    size_t dataBytes = (size_t)nnz * 20 + (size_t)numRows * 8;

    double origBW = dataBytes / (origAvgUs * 1e-6) / 1e9;
    double rcmBW = dataBytes / (rcmAvgUs * 1e-6) / 1e9;

    double peakBW = 1008;  // RTX 4090 peak bandwidth GB/s

    printf("  Original: %.1f us/iter, %.1f GB/s (%.1f%% utilization)\n",
           origAvgUs, origBW, origBW/peakBW*100);
    printf("  RCM:      %.1f us/iter, %.1f GB/s (%.1f%% utilization)\n",
           rcmAvgUs, rcmBW, rcmBW/peakBW*100);
    printf("  Speedup:  %.3fx\n\n", origAvgUs/rcmAvgUs);
...
int main(int argc, char** argv) {
    printf("========================================\n");
    printf("  RTX 4090 RCM Comparison Test\n");
    printf("  Using cudaEvent for precise timing\n");
    printf("========================================\n\n");

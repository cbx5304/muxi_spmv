/**
 * @file test_cache_optimizations.cu
 * @brief Test cache-specific optimizations for random access patterns
 *
 * Optimizations tested:
 * 1. Different cache hints (__ldg, __ldcg, __ldca, __ldcs, __ldlu)
 * 2. Cache configuration variants
 * 3. Texture memory for x-vector
 * 4. Software-managed cache with shared memory
 */
...
// Baseline with __ldg (read-only cache)
template<int BLOCK_SIZE>
__global__ void spmv_ldg(int numRows, const int* __restrict__ rowPtr,
...
// Cache global (cache at global level - default)
template<int BLOCK_SIZE>
__global__ void spmv_ldcg(int numRows, const int* __restrict__ rowPtr,
                           const int* __restrict__ colIdx,
                           const float* __restrict__ values,
                           const float* __restrict__ x, float* __restrict__ y) {
    int warpId = threadIdx.x / WARP_SIZE;
    ...
    for (; idx + 4 < rowEnd; idx += 8) {
        sum0 += values[idx] * __ldcg(&x[colIdx[idx]]);
        sum1 += values[idx + 4] * __ldcg(&x[colIdx[idx + 4]]);
    }
    for (; idx < rowEnd; idx += 4) {
        sum0 += values[idx] * __ldcg(&x[colIdx[idx]]);
    }
...
// Cache all (cache at all levels)
template<int BLOCK_SIZE>
__global__ void spmv_ldca(int numRows, const int* __restrict__ rowPtr,
                           const int* __restrict__ colIdx,
                           const float* __restrict__ values,
                           const float* __restrict__ x, float* __restrict__ y) {
    int warpId = threadIdx.x / WARP_SIZE;
    ...
    for (; idx + 4 < rowEnd; idx += 8) {
        sum0 += values[idx] * __ldca(&x[colIdx[idx]]);
        sum1 += values[idx + 4] * __ldca(&x[colIdx[idx + 4]]);
    }
    for (; idx < rowEnd; idx += 4) {
        sum0 += values[idx] * __ldca(&x[colIdx[idx]]);
    }
...
// Cache streaming (bypass L1, stream to L2)
template<int BLOCK_SIZE>
__global__ void spmv_ldcs(int numRows, const int* __restrict__ rowPtr,
                           const int* __restrict__ colIdx,
                           const float* __restrict__ values,
                           const float* __restrict__ x, float* __restrict__ y) {
    int warpId = threadIdx.x / WARP_SIZE;
    ...
    for (; idx + 4 < rowEnd; idx += 8) {
        sum0 += values[idx] * __ldcs(&x[colIdx[idx]]);
        sum1 += values[idx + 4] * __ldcs(&x[colIdx[idx + 4]]);
    }
    for (; idx < rowEnd; idx += 4) {
        sum0 += values[idx] * __ldcs(&x[colIdx[idx]]);
    }
...
// Last use hint
template<int BLOCK_SIZE>
__global__ void spmv_ldlu(int numRows, const int* __restrict__ rowPtr,
                           const int* __restrict__ colIdx,
                           const float* __restrict__ values,
                           const float* __restrict__ x, float* __restrict__ y) {
    int warpId = threadIdx.x / WARP_SIZE;
    ...
    for (; idx + 4 < rowEnd; idx += 8) {
        sum0 += values[idx] * __ldlu(&x[colIdx[idx]]);
        sum1 += values[idx + 4] * __ldlu(&x[colIdx[idx + 4]]);
    }
    for (; idx < rowEnd; idx += 4) {
        sum0 += values[idx] * __ldlu(&x[colIdx[idx]]);
    }
...
// No cache hints
template<int BLOCK_SIZE>
__global__ void spmv_no_hint(int numRows, const int* __restrict__ rowPtr,
                              const int* __restrict__ colIdx,
                              const float* __restrict__ values,
                              const float* __restrict__ x, float* __restrict__ y) {
    int warpId = threadIdx.x / WARP_SIZE;
...
int main(int argc, char** argv) {
    printf("=== Cache Optimization Test ===\n");
    printf("WARP_SIZE = %d\n", WARP_SIZE);
    ...
    t = runTest(spmv_no_hint<512>, gridSize, blockSize, iterations, matrix, d_x, d_y);
    printResult("No Hint", t);

    t = runTest(spmv_ldg<512>, gridSize, blockSize, iterations, matrix, d_x, d_y);
    printResult("__ldg (Read-Only)", t);

    t = runTest(spmv_ldcg<512>, gridSize, blockSize, iterations, matrix, d_x, d_y);
    printResult("__ldcg (Cache Global)", t);

    t = runTest(spmv_ldca<512>, gridSize, blockSize, iterations, matrix, d_x, d_y);
    printResult("__ldca (Cache All)", t);

    t = runTest(spmv_ldcs<512>, gridSize, blockSize, iterations, matrix, d_x, d_y);
    printResult("__ldcs (Cache Stream)", t);

    t = runTest(spmv_ldlu<512>, gridSize, blockSize, iterations, matrix, d_x, d_y);
    printResult("__ldlu (Last Use)", t);

    printf("\n=== Analysis ===\n");
    printf("Testing different cache hints for x-vector access.\n");
    printf("For random access patterns, all cache hints should converge.\n");
